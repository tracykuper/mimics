...
def run_GENRE(old_biomass,metabolite_concentrations,index,models, metabolic_state,rxns,metabolite_ids,v_patch,dt_rxn,dt_growth):
    ...
    try:
        if old_biomass > 0: # CHECK AGENT BIOMASS IS ABOVE ZERO
    ...
    return new_biomass, tuple(updated_metabolite_concentrations), updated_index,growth_rate, tuple(rxn_flux_all)
...
def run_MIMICS(ncpus, num_dt,models,media,rxns, job_num,num_met_gas,initial_gas_concentrations, D_gas, num_gas_step, num_met_carbon,initial_carbon_concentrations, D_carbon,num_carbon_step, metabolite_ids, v_patch, dt_rxn, dt_growth,initial_biomass,max_biomass,dead_state,output_dir):
   ...
   # CONSTRAIN METABOLIC MODELS ON NUTRIENT MEDIA
    ...
    for t in range(num_dt):
        gateway.entry_point.run_ABM(initial_biomass,max_biomass,int(dead_state)) ## RUN AGENT-BASED MODEL
        ...
        a = np.array((biomass_values,metabolite_concentrations, index_values, [models]*pos_count, metabolic_states,[rxns]*pos_count,[metabolite_ids]*pos_count,[v_patch]*pos_count,[dt_rxn]*pos_count,[dt_growth]*pos_count),dtype=object).T
        ...
        gateway.entry_point.setGrowthRateFromPython(growth_rates_total, index_values_for_java,int(dead_state))
        ...
        if  t >0:
            gateway.Save_cell_info(int(t), int(job_num),output_dir)
        times_save=[0,25,50,91,92,93,94,95,96,97,98,99] # DEFINE TIMES TO SAVE METABOLITE CONCENTRATIONS
        if  t in times_save:
        ...
        times_save=[15,30,45,60,75,98,99] # DEFINE TIMES TO SAVE REACTION FLUXES
...
if __name__ == '__main__':
    ...
    abm_parameters_filename = 'insert directory + XLSX filename of ABM parameter values'
    ...
    run_MIMICS(ncpus,int(num_dt),models,media,rxns,int(job_num), num_met_gas,initial_gas_concentrations_java, D_gas_java, num_gas_step, num_met_carbon,initial_carbon_concentrations_java, D_carbon_java,num_carbon_step,metabolite_ids,v_patch, dt_rxn, dt_growth,initial_biomass,max_biomass,dead_state,output_dir);
