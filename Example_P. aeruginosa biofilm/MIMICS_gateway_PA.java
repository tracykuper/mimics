...
public class MIMICS_gateway_PA {
    public static MIMICS_PA bis;
    public MIMICS_gateway_PA() {
    }

    // FUNCTION TO TO RETURN MIMICS CLASS (EXECUTED FROM PYTHON)
    public MIMICS_PA getStack() {
        return bis;
    }
    ...
    public static void run_model0(int xdim, int ydim, int zdim, int b0, double initial_biomass,double max_biomass, int num_met_gas, int num_met_carbon, double[] initial_gas_concentrations, double[] initial_carbon_concentrations) {
        bis = new MIMICS_PA(xdim, ydim, zdim); //CREATE MIMICS OBJECT AND SET X,Y,Z DIMENSIONS
    ...
    public static void run_ABM(double initial_biomass, double max_biomass,int dead_state,double lag_phase, int time_step) {
        bis.StepCells(initial_biomass,max_biomass,dead_state,lag_phase,time_step);
    }
    ...
    public double[] getBiomassFromHal() {
        ...
        for (Cell3D_PA cell : bis.AllAgents()) {
    ...
    public void setGrowthRateFromPython(double[] growth_rate, int[] index, int dead_state, double lag_phase, int time_step) {
        ...
        for (int g = 0; g < index.length; g++) {
            ...
            for (Cell3D_PA cell : bis.AllAgents()) {
                if (cell.index == index[g]) { // CHECK IF AGENT INDEX IS EQUAL TO INDEX FROM PYTHON
                    ...
                    // ASSIGN DEAD CELL STATE, ONLY WHEN CELLS ARE OUT OF THE LAG PHASE
                    if ((cell.growth_rate ==0) & (time_step >= (int) Math.round(lag_phase))){
    ...
    public void setBiomassFromPython(double[] mass, int[] index) {
        ...
        for (int g = 0; g < index.length; g++) {
            // ITERATE OVER AGENTS IN ABM
            for (Cell3D_PA cell : bis.AllAgents()) { // CHECK IF AGENT INDEX IS EQUAL TO INDEX FROM PYTHON
    ...
    public static void main(String[] args) throws IOException, InterruptedException {
        GatewayServer gatewayServer = new GatewayServer(new MIMICS_gateway_PA()); // INITIALIZE JAVA PY4J GATEWAY SERVER
        gatewayServer.start(); // START JAVA GATEWAY SERVER FOR PYTHON ACCESS
        System.out.println("PY4J Gateway Server Started");

//        // USE  CODE BELOW FOR MIMICS GATEWAY SERVER TO AUTOMATICALLY RUN MIMICS PYTHON FILE

//        String command = "python agents_FBA_PA.py"; // PYTHON FILE TO OPTIMIZE EACH AGENT'S GENRE
//        //String command = "/Users/tracykuper/opt/anaconda3/envs/netlogo/bin/python3 /Users/tracykuper/Desktop/HAL-master-2/MIMICS/agents_FBA_PA.py"; // PYTHON FILE TO OPTIMIZE EACH AGENT'S GENRE
